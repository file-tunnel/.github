...
The selectable workflow template is standalone and uses immutable external Action pins. A repository that calls `.github/workflows/reusable-policy.yml` directly must pin the call to a reviewed full commit SHA of `file-tunnel/.github`; do not use a mutable branch or tag for enforcement.
